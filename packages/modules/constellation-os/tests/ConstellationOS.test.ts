...
class MockStorage implements IStorage {
  ...
  async query<T>(table: string, filter?: Partial<T>): Promise<T[]> {
  ...
  async update<T extends { id: string }>(table: string, record: T): Promise<void> {
    const records = this.store.get(table) ?? [];
...
describe('ConstellationOS', () => {
  ...
  let constellationOS: ConstellationOS;
  ...
  describe('CRUD Operations', () => {
    ...
    describe('updateConstellation', () => {
      ...
      it('throws error when constellation not found', async () => {
        await expect(
          constellationOS.updateConstellation('nonexistent', { name: 'Test' })
        ).rejects.toThrow('Constellation not found: nonexistent');
      });
      ...
      it('throws error when storage is not available', async () => {
        ...
        await expect(
          nostorage.updateConstellation('test-id', { name: 'Test' })
        ).rejects.toThrow('Storage not available');
      });
    });
  ...
  describe('Statistics', () => {
    describe('getStats', () => {
      it('calculates stats from session and ritual logs', async () => {
        ...
        await storage.insert<SessionLog>('session_logs', {
          sessionId: 'session-1',
          constellationId,
        ...
        // Add a ritual log
        await storage.insert<RitualLog>('ritual_logs', {
  ...
  describe('Event Handlers', () => {
    describe('SessionEnded', () => {
      it('logs session when SessionEnded event is emitted', async () => {
        ...
        const logs = await storage.query<SessionLog>('session_logs', { constellationId });
      ...
      it('does not log session without constellationId', async () => {
        ...
        const logs = await storage.query<SessionLog>('session_logs');
    ...
    describe('RitualCompleted', () => {
      it('handles RitualCompleted event', async () => {
        const payload: RitualCompletedPayload = {
          ritualId: 'morning-anchor',
          sessionId: 'test-session',
          totalDuration: 300,
